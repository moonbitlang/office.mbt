...
if [[ $# -ne 0 ]]; then
  echo "Usage: scripts/check_parity_preflight_matrix_smoke_contract_13.sh [--json]"
  exit 2
fi
...
if json_proc.returncode != 0:
    ...
    if payload is not None:
        ...
        required_checks = {
            ...
            "required_pattern::scripts/check_parity_preflight_matrix_smoke_contract_12.sh",
...
invalid_proc = subprocess.run(
    ["scripts/check_parity_preflight_matrix_smoke_contract_12.sh", "--bad"],
    capture_output=True,
    text=True,
    check=False,
)
...
invalid_ok = (
    ...
    and "Usage: scripts/check_parity_preflight_matrix_smoke_contract_12.sh [--json]"
    in invalid_output
)
add_result(
