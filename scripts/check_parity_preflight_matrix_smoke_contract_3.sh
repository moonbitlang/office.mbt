...
if [[ $# -ne 0 ]]; then
  echo "Usage: scripts/check_parity_preflight_matrix_smoke_contract_3.sh [--json]"
  exit 2
fi
...
if json_proc.returncode != 0:
    ...
    if payload is not None:
        ...
        required_checks = {
            ...
            "required_pattern::scripts/check_parity_preflight_matrix_smoke_contract_2.sh",
...
invalid_proc = subprocess.run(
    ["scripts/check_parity_preflight_matrix_smoke_contract_2.sh", "--bad"],
    capture_output=True,
    text=True,
    check=False,
)
...
invalid_ok = (
    ...
    and "Usage: scripts/check_parity_preflight_matrix_smoke_contract_2.sh [--json]"
    in invalid_output
)
add_result(
