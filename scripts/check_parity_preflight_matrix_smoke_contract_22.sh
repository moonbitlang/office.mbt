...
if [[ $# -ne 0 ]]; then
  echo "Usage: scripts/check_parity_preflight_matrix_smoke_contract_22.sh [--json]"
  exit 2
fi
...
invalid_proc = subprocess.run(
    ["scripts/check_parity_preflight_matrix_smoke_contract_21.sh", "--bad"],
    capture_output=True,
    text=True,
    check=False,
)
...
invalid_ok = (
    ...
    and "Usage: scripts/check_parity_preflight_matrix_smoke_contract_21.sh [--json]"
    in invalid_output
)
add_result(
