...
if [[ $json_mode -eq 1 ]]; then
  if [[ $compact_mode -eq 1 ]]; then
    printf '{"wrapper_preflight":"%s","env_helper_preflight":"%s","preflight_status_helper_preflight":"%s","gate_toggle_consistency_preflight":"%s","gate_toggle_contract_preflight":"%s","gate_show_toggle_contract_preflight":"%s","preflight_matrix_smoke_preflight":"%s","docs_preflight":"%s","env":{"SKIP_PARITY_WRAPPER_PREFLIGHT":"%s","SKIP_PARITY_ENV_HELPER_PREFLIGHT":"%s","SKIP_PARITY_PREFLIGHT_STATUS_HELPER_PREFLIGHT":"%s","SKIP_PARITY_GATE_TOGGLE_PREFLIGHT":"%s","SKIP_PARITY_GATE_TOGGLE_CONTRACT_PREFLIGHT":"%s","SKIP_PARITY_GATE_SHOW_TOGGLE_PREFLIGHT":"%s","SKIP_PARITY_PREFLIGHT_MATRIX_SMOKE_PREFLIGHT":"%s","SKIP_PARITY_DOCS_PREFLIGHT":"%s"}}\n' \
      "$wrapper_status" \
      "$env_helper_status" \
