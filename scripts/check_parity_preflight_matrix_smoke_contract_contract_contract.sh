...
if [[ $# -ne 0 ]]; then
  echo "Usage: scripts/check_parity_preflight_matrix_smoke_contract_contract_contract.sh [--json]"
  exit 2
fi
...
if json_proc.returncode != 0:
    add_result(
        "matrix_smoke_contract_contract_json_invocation",
        False,
    ...
    try:
        ...
        add_result(
            "matrix_smoke_contract_contract_json_parse",
            False,
            f"invalid json: {exc}",
        )
        payload = None
...
invalid_proc = subprocess.run(
    ["scripts/check_parity_preflight_matrix_smoke_contract_contract.sh", "--bad"],
    capture_output=True,
    text=True,
    check=False,
)
...
invalid_ok = (
    ...
    and "Usage: scripts/check_parity_preflight_matrix_smoke_contract_contract.sh [--json]"
    in invalid_output
)
add_result(
...
if json_mode:
    ...
    if failed:
        print("Parity preflight matrix smoke contract-contract checker contract check failed.")
    else:
        print("Parity preflight matrix smoke contract-contract checker contract check passed.")
