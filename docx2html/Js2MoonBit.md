...
- Mammoth's style-map HTML target grammar is strict even when the eventual MoonBit data structure is easy to build leniently. Reject trailing whitespace or junk, malformed attributes, missing arrows, missing spaces around `>`, and option order such as `:separator('x'):fresh`; `:fresh:separator('x')` is the accepted order.
...
- `includeDefaultStyleMap=false` disables Mammoth's built-in style-map lines but not embedded style maps. Embedded maps still apply unless `includeEmbeddedStyleMap=false` is also set; list fixtures then fall back to plain paragraphs and can surface unrecognised-style warnings.
