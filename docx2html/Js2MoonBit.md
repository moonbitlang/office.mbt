...
- Mammoth's style-map HTML target grammar is strict even when the eventual MoonBit data structure is easy to build leniently. Reject trailing whitespace or junk, malformed attributes, missing spaces around `>`, and option order such as `:separator('x'):fresh`; `:fresh:separator('x')` is the accepted order.
