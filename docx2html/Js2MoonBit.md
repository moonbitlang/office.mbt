...
- Raw-text extraction follows Mammoth's document model, not rendered text: text and tabs are emitted, paragraphs add two newlines, and line/page/column break elements contribute no text. Mammoth image reads are lazy, so raw text must not force image data reads or surface image diagnostics.
