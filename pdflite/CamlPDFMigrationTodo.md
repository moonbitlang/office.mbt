...
- Warning 74 public documentation cleanup: complete; 0 native diagnostics
  ...
  role-map/auto-artifact state, and cpdfdraw
  ...
  document-info and XMP text-output reporting, XMP/document info JSON reporting,
  cpdfmetadata source-spelled encode-output, print-metadata, and XML-tree
  aliases, namespace aliases, and namespace/get-data/XML-tree helper parity,
  ...
  source string wrapper, cpdfxobject source string result wrapper, cpdf page
...
- [x] ~~Continue `cpdfmetadata` source/API parity from
  ...
  `output_info` report. The wrapper preserves cpdf's document-info line order,
  `Version`/`Pages` rendering, info-string encoding selection,
  catalog/viewer-preference/open-action fields, capitalized MarkInfo/XFA output,
  and six-decimal page-box formatting with `various` detection while returning
  UTF-8 bytes for library callers. Coverage extends the existing info report
  test across populated info fields, viewer preferences, open action text,
  AcroForm/XFA, MarkInfo booleans, page-box unit conversion, stripped text
  encoding, and wrapper parity. Validation on MoonBit 0.9.3: `moon check
  --target native --warn-list +73` passes with the known `markdown/cmd` future
  notice; `moon test --target native pdf_metadata_test.mbt --filter
  '*info_json*'`, `moon test --target wasm-gc pdf_metadata_test.mbt --filter
  '*info_json*'`, and `moon test --target js pdf_metadata_test.mbt --filter
  '*info_json*'` each report 2/2; `moon test --target native
  pdf_metadata_test.mbt` reports 21/21; `moon fmt && moon info` are clean;
  `moon check --target all --warn-list +73` passes with the known `markdown/cmd`
...
- [x] ~~Continue `cpdfmetadata` source/API parity from
  ...
  `pdf_pdfxid`, `pdf_pdfe`, `pdf_pdfuaid`, and `pdf_pdfvtid` over the existing
  `pdf_metadata_*` namespace constants. Coverage folds the aliases into the
  existing XML metadata namespace lookup test. Validation on MoonBit 0.9.3:
  `moon check --target native --warn-list +73` passes with the known
  `markdown/cmd` future notice; `moon test --target native
  pdf_metadata_test.mbt --filter '*get_data_for_bytes*'`, `moon test --target
  wasm-gc pdf_metadata_test.mbt --filter '*get_data_for_bytes*'`, and `moon
  test --target js pdf_metadata_test.mbt --filter '*get_data_for_bytes*'`
  report 1/1 each; `moon fmt`, `moon info`, and `moon check --target all
...
- [x] ~~Continue `cpdfaddtext` source/API parity from
  ...
  add-text fill/stroke colour operator helpers. Coverage folds the aliases into
  the existing fill and stroke colour mapping tests. Validation on MoonBit
  0.9.3: `moon check --target native --warn-list +73` passes with the known
  `markdown/cmd` future notice; `moon test --target native
  pdf_addtext_test.mbt`, `moon test --target wasm-gc pdf_addtext_test.mbt`,
  and `moon test --target js pdf_addtext_test.mbt` report 117/117 each; `moon
  fmt`, `moon info`, and `moon check --target all --warn-list +73` pass with
  only the existing `markdown/cmd` future notice; full `moon test --target
  native` reports 2414/2414.~~
...
- [x] ~~Continue `cpdfxobject` source/API parity from
  ...
  resource name while retaining the existing typed `PdfName` wrapper. Coverage
  asserts the returned source string, selected-page XObject resource insertion,
  and unchanged unselected pages. Validation on MoonBit 0.9.3: `moon check
  --target native --warn-list +73` passes with the known `markdown/cmd` future
  notice; `moon test --target native pdf_xobject_test.mbt`, `moon test --target
  wasm-gc pdf_xobject_test.mbt`, and `moon test --target js
  pdf_xobject_test.mbt` report 4/4 each; `moon fmt`, `moon info`, and `moon
  check --target all --warn-list +73` pass with only the existing
  `markdown/cmd` future notice; full `moon test --target native` reports
  2414/2414, full `moon test --target wasm-gc` reports 2072/2072, and full
  `moon test --target js` reports 2072/2072.~~
...
- [x] ~~Continue `cpdfdrawcontrol` source/API parity from
  ...
  the existing `PdfDrawControl::add_jpeg_data`, `add_png_data`, and
  `add_jpeg2000_data` methods. This exposes cpdf's image command names for the
  supplied-byte branch while keeping filesystem path reads in the native
  `async_io` package; `pdf_addjpeg` accepts and ignores `path_to_im` for source
  API parity because pdflite parses JPEG dimensions in-process. Coverage
  compares wrapper-built image XObjects with method-built JPEG, PNG, and
  JPEG2000 ops and pins parser error propagation through the wrappers.
  Validation on MoonBit 0.9.3: `moon check --target native --warn-list +73`
  passes with the known `markdown/cmd` future notice; `moon test --target
  native pdf_draw_test.mbt --filter '*image data helpers*'`, `moon test
  --target wasm-gc pdf_draw_test.mbt --filter '*image data helpers*'`, and
  `moon test --target js pdf_draw_test.mbt --filter '*image data helpers*'`
  report 2/2 each; `moon fmt`, `moon info`, and `moon check --target all
  --warn-list +73` pass with only the existing `markdown/cmd` future notice;
...
- [x] ~~Continue `cpdftweak` source/API parity for reveal-hidden-text from
  ...
  order while preserving the existing document-first wrapper. Coverage compares
  the source-order alias against the existing selected-page rewrite behavior.
  Validation on MoonBit 0.9.3: `moon check --target native --warn-list +73`
  passes with the known `markdown/cmd` future notice; `moon test --target native
  pdf_tweak_test.mbt` reports 27/27; `moon fmt`, `moon info`, and `moon check
  --target all --warn-list +73` pass with only the existing `markdown/cmd`
  future notice; full native `moon test --target native` reports 2407/2407.~~
...
- [x] ~~Continue source-port hardening with a content-stream performance slice:
  ...
  +73` passes with the known notice; `moon test --target native` reports
  2428/2428.~~
- [x] ~~Continue source-port hardening with a core byte-concat performance
  slice: `pdf_bytes_concat_views` now precomputes the exact owned-byte length
  and fills by index, and AESV3 password/key derivation now reuses that shared
  helper instead of maintaining a private grow-by-push concat loop. Validation
  on MoonBit compiler 0.9.3: `moon check --target native --warn-list +73`
  passes with the known notice; focused PdfBytes concat reports 1/1;
  `moon test --target native pdflite_test.mbt` reports 9/9; focused AESV3
...
- [x] ~~Continue source-port hardening with an ASCIIHex codec performance slice:
  ...
  `moon check --target all --warn-list +73` passes with the known notice; `moon
  test --target native` reports 2428/2428.~~
- [x] ~~Continue source-port hardening with an ASCII85 codec performance slice:
  `pdf_ascii85_encode_view` now allocates one upper-bound output buffer, fills
  full/partial/zero-compressed ASCII85 groups by index, and returns the written
  prefix instead of growing a byte array per output group, preserving full,
  partial, zero-compressed, and empty `~>` outputs. Validation on MoonBit
  compiler 0.9.3: `moon check --target native --warn-list +73` passes with the
...
- [x] ~~Continue source-port hardening with an ASCIIHex decode performance
  ...
  reports 2428/2428.~~
- [x] ~~Continue source-port hardening with a RunLength codec performance slice:
  `pdf_run_length_encode_view` now allocates the worst-case encoded byte buffer,
  flushes literal chunks from the borrowed input by index, writes repeated runs
  and the `128` EOD marker in place, and returns the written prefix instead of
  growing output and literal arrays. Validation on MoonBit compiler 0.9.3:
...
- [x] ~~Continue source-port hardening with a PNG predictor decode performance
  slice: `pdf_decode_png_predictor` now precomputes decoded row output length,
  allocates one exact byte buffer, and `pdf_decode_png_push_scanline` fills each
  decoded scanline by index instead of growing the output array, preserving PNG
  filter rows, short-row zero padding, Paeth behavior, unsupported-filter
  errors, and empty input. Validation on MoonBit compiler 0.9.3: `moon check
...
- [x] ~~Continue source-port hardening with an ASCII85 decode performance slice:
  ...
  --warn-list +73` passes with the known notice; `moon test --target native`
  reports 2428/2428.~~
- [x] ~~Continue source-port hardening with a writer hex-string performance
  slice: `pdf_write_hex_string` now allocates the exact `<...>` byte length and
  fills lowercase hex pairs by index instead of growing a byte array through the
  private writer helper, while preserving lowercase output and the empty `<>`
  case. Validation on MoonBit compiler 0.9.3: `moon check --target native
  --warn-list +73` passes with the known notice; focused writer hex-string tests
...
- [x] ~~Continue source-port hardening with a standard-security password
  ...
  changes.~~
- [x] ~~Continue source-port hardening with an ARC4 stream output performance
  slice: `pdf_arc4_crypt` now allocates the exact encrypted/decrypted output
  length from the borrowed data length and fills stream bytes by index instead
  of growing the output array, preserving standard ARC4 vectors, symmetric
  decrypt-by-reapply behavior, empty plaintext output, empty-key errors,
  standard-security file-key use, and public crypt/decrypt behavior. Validation
  on MoonBit compiler 0.9.3: `moon check --target native --warn-list +73`
  passes with the known notice; focused ARC4 tests report 3/3; `moon test
  --target native crypt_core/pdf_crypt_primitives_wbtest.mbt` reports 10/10;
  `moon test --target native pdf_crypt_wbtest.mbt` reports 26/26; `moon test
  --target native pdf_crypt_test.mbt` reports 43/43; `moon check --target all
  --warn-list +73` passes with the known notice; `moon test --target native`
  reports 2429/2429; `moon info && moon fmt` are clean with no `.mbti`
  changes.~~
- [x] ~~Continue source-port hardening with an MD5 digest output performance
  slice: `pdf_md5_digest` now allocates the exact 16-byte digest result and
  fills each little-endian state word by index instead of growing the output
  array, preserving RFC 1321 vectors, standard-security file-key derivation,
  and public password crypt/decrypt behavior. Validation on MoonBit compiler
  0.9.3: `moon check --target native --warn-list +73` passes with the known
  notice; focused MD5 vector tests report 1/1; `moon test --target native
  ...
  reports 2429/2429; `moon info && moon fmt` are clean with no `.mbti`
  changes.~~
- [x] ~~Continue source-port hardening with a SHA digest serialization
  performance slice: `pdf_sha512_digest_with_state` now allocates the exact
  SHA-384/SHA-512 digest byte length and writes each big-endian state word by
  index instead of growing the result array, preserving FIPS vectors, 48/64-byte
  digest lengths, AESV3 password hashing, and public crypt/decrypt behavior.
  Validation on MoonBit compiler 0.9.3: `moon check --target native
  --warn-list +73` passes with the known notice; focused SHA-384/SHA-512 vector
  tests report 1/1; `moon test --target native
  crypt_core/pdf_crypt_primitives_wbtest.mbt` reports 10/10; `moon test
  --target native pdf_crypt_wbtest.mbt` reports 26/26; `moon test --target
  native pdf_crypt_test.mbt` reports 43/43; `moon check --target all
  --warn-list +73` passes with the known notice; `moon test --target native`
...
- [x] ~~Continue source-port hardening with an AES-CBC decrypt performance
  ...
  reports 2429/2429; `moon info && moon fmt` are clean with no `.mbti`
  changes.~~
- [x] ~~Continue source-port hardening with an AES-ECB decrypt performance
  slice: `pdf_aes_ecb_decrypt` now allocates the exact pre-padding plaintext
  length and copies decrypted blocks by index instead of growing the output
  array, preserving no-padding decrypt vectors, PKCS padding removal, empty
  input, invalid-length errors, and public crypt/decrypt behavior. Validation
  on MoonBit compiler 0.9.3: `moon check --target native --warn-list +73`
  passes with the known notice; focused AES-ECB tests report 1/1; `moon test
  --target native crypt_core/pdf_crypt_primitives_wbtest.mbt` reports 10/10;
  `moon test --target native pdf_crypt_wbtest.mbt` reports 26/26; `moon test
  --target native pdf_crypt_test.mbt` reports 43/43; `moon check --target all
  --warn-list +73` passes with the known notice; `moon test --target native`
  reports 2429/2429; `moon info && moon fmt` are clean with no `.mbti`
  changes.~~
- [x] ~~Continue source-port hardening with an AES-ECB encrypt performance
  slice: `pdf_aes_ecb_encrypt` now allocates the exact ciphertext length and
  copies encrypted blocks by index instead of growing the output array,
  preserving AES known vectors, empty input, invalid-length errors, and public
  crypt/decrypt behavior. Validation on MoonBit compiler 0.9.3: `moon check
  --target native --warn-list +73` passes with the known notice; focused
  AES-ECB encrypt/decrypt tests report 2/2; `moon test --target native
  crypt_core/pdf_crypt_primitives_wbtest.mbt` reports 10/10; `moon test
  --target native pdf_crypt_wbtest.mbt` reports 26/26; `moon test --target
  native pdf_crypt_test.mbt` reports 43/43; `moon check --target all
  --warn-list +73` passes with the known notice; `moon test --target native`
  reports 2429/2429; `moon info && moon fmt` are clean with no `.mbti`
  changes.~~
- [x] ~~Continue source-port hardening with an AES-CBC encrypt performance
  slice: `pdf_aes_cbc_encrypt_with_iv` now allocates the exact IV-prefixed
  ciphertext length, writes the IV and encrypted blocks by index, and removes
  the old grow-by-push block append helper, preserving short and exact-block
  PKCS padding, AESV2/AESV3 object encryption, and public crypt/decrypt
  behavior. Validation on MoonBit compiler 0.9.3: `moon check --target native
  --warn-list +73` passes with the known notice; focused AES-CBC tests report
  1/1; `moon test --target native crypt_core/pdf_crypt_primitives_wbtest.mbt`
  reports 10/10; `moon test --target native pdf_crypt_wbtest.mbt` reports
  26/26; `moon test --target native pdf_crypt_test.mbt` reports 43/43; `moon
  check --target all --warn-list +73` passes with the known notice; `moon test
  --target native` reports 2429/2429; `moon info && moon fmt` are clean with no
...
- [x] ~~Continue source-port hardening with an MD5 padded-message performance
  ...
  reports 2429/2429; `moon info && moon fmt` are clean with no `.mbti`
  changes.~~
- [x] ~~Continue source-port hardening with an ARC4 state initialization
  performance slice: `pdf_arc4_crypt` now allocates the fixed 256-entry ARC4
  permutation state and fills it by index instead of growing the state array,
  preserving standard ARC4 vectors, symmetric decrypt-by-reapply behavior,
  empty-key errors, object encryption, and public crypt/decrypt behavior.
  Validation on MoonBit compiler 0.9.3: `moon check --target native
  --warn-list +73` passes with the known notice; focused ARC4 tests report
  3/3; `moon test --target native crypt_core/pdf_crypt_primitives_wbtest.mbt`
  reports 10/10; `moon test --target native pdf_crypt_wbtest.mbt` reports
  26/26; `moon test --target native pdf_crypt_test.mbt` reports 43/43; `moon
  check --target all --warn-list +73` passes with the known notice; `moon test
  --target native` reports 2429/2429; `moon info && moon fmt` are clean with no
  `.mbti` changes.~~
- [x] ~~Continue source-port hardening with a standard file-key derivation
  performance slice: `pdf_encryption_file_key` now precomputes the exact
  digest-input byte length, copies padded password, owner entry, permissions,
  file ID, and optional no-metadata marker into one fixed buffer by index, and
  preserves R2/R3/R4 file-key vectors, MD5 padding boundary behavior, password
  authentication, and public crypt/decrypt behavior. Validation on MoonBit
  compiler 0.9.3: `moon check --target native --warn-list +73` passes with the
  known notice; focused standard file-key and MD5-boundary tests report 2/2;
  `moon test --target native crypt_core/pdf_crypt_primitives_wbtest.mbt`
  reports 10/10; `moon test --target native pdf_crypt_wbtest.mbt` reports
  26/26; `moon test --target native pdf_crypt_test.mbt` reports 43/43; `moon
  check --target all --warn-list +73` passes with the known notice; `moon test
  --target native` reports 2429/2429; `moon info && moon fmt` are clean with no
  `.mbti` changes.~~
- [x] ~~Continue source-port hardening with a user-password entry performance
  slice: `pdf_make_user_password_entry` now builds the revision 3/4 MD5 input
  from one exact padding-plus-file-ID buffer, writes the encrypted digest into
  a fixed zero-filled `/U` entry buffer, and removes the grow-by-push byte-view
  helper. This preserves CamlPDF `/U` vectors, MD5 padding boundary behavior,
  password authentication, and public crypt/decrypt behavior. Validation on
  MoonBit compiler 0.9.3: `moon check --target native --warn-list +73` passes
  with the known notice; focused user-entry and MD5-boundary tests report 2/2;
  `moon test --target native crypt_core/pdf_crypt_primitives_wbtest.mbt`
  reports 10/10; `moon test --target native pdf_crypt_wbtest.mbt` reports
  26/26; `moon test --target native pdf_crypt_test.mbt` reports 43/43; `moon
  check --target all --warn-list +73` passes with the known notice; `moon test
  --target native` reports 2429/2429; `moon info && moon fmt` are clean with no
  `.mbti` changes.~~
- [x] ~~Continue source-port hardening with an AES-CBC padding/encrypt
  performance slice: `pdf_aes_cbc_encrypt_with_iv` now streams full plaintext
  blocks and the final PKCS padding block directly into the exact IV-prefixed
  ciphertext buffer, removing the intermediate padded-block array while
  preserving short and exact-block padding, AESV2/AESV3 object encryption, and
  public crypt/decrypt behavior. Validation on MoonBit compiler 0.9.3: `moon
  check --target native --warn-list +73` passes with the known notice; focused
  AES-CBC tests report 1/1; `moon test --target native
  crypt_core/pdf_crypt_primitives_wbtest.mbt` reports 10/10; `moon test
...
- [x] ~~Continue source-port hardening with a content unknown-operator fallback
  ...
  operand separators, allocates one fixed byte buffer, and fills operand and
  operator bytes by index instead of growing the fallback output array,
  preserving comment skipping, literal-operand fallback, invalid known-operator
  preservation, and normal content rendering. Validation on MoonBit compiler
  0.9.3: `moon check --target native --warn-list +73` passes with the known
  notice; focused unknown-operator content tests report 3/3; `moon test
  --target native pdf_content_test.mbt` reports 58/58; `moon check --target all
  --warn-list +73` passes with the known notice; `moon test --target native`
  reports 2429/2429; `moon info && moon fmt` are clean with no `.mbti`
  changes.~~
...
- [x] ~~Continue source-port hardening with a metadata XMP element rewrite
  ...
  tag lengths before copying/replacing XML element content, preserving
  self-closing XMP fields, existing element replacement, escaped XML text,
  Info/XMP synchronization, and invalid value errors. Validation on MoonBit
  compiler 0.9.3: `moon check --target native --warn-list +73` passes with the
  known notice; focused XMP synchronization tests report 1/1; `moon test
...
- [x] ~~Continue source-port hardening with a bookmark JSON page-selection
  ...
  the array, preserving full-document bookmark JSON rows, selected target
  rewriting, colour/flag output, and JSON blob rendering. Validation on MoonBit
  compiler 0.9.3: `moon check --target native --warn-list +73` passes with the
  known notice; focused bookmark JSON tests report 1/1; `moon test --target
  native pdf_bookmark_test.mbt` reports 28/28; `moon check --target all
  --warn-list +73` passes with the known notice; `moon test --target native`
  reports 2429/2429.~~
...
- [x] ~~Continue source-port hardening with a bookmark stripped-listing
  ...
  growing the output array, preserving non-ASCII filtering, newline/quote
  escaping, selected-page bookmark listings, and destination rendering.
  Validation on MoonBit compiler 0.9.3: `moon check --target native --warn-list
  +73` passes with the known notice; focused stripped bookmark listing tests
  report 1/1; `moon test --target native pdf_bookmark_test.mbt` reports 28/28;
  `moon check --target all --warn-list +73` passes with the known notice; `moon
...
- [x] ~~Continue source-port hardening with a bookmark import escape-fixup
  ...
  and `\n` escapes by index instead of growing the output array, preserving
  unknown escape literals, optional destination parsing, open flags, named
  targets, and read-back bookmark structure. Validation on MoonBit compiler
  0.9.3: `moon check --target native --warn-list +73` passes with the known
  notice; focused bookmark import tests report 1/1; `moon test --target native
  pdf_bookmark_test.mbt` reports 27/27; `moon check --target all --warn-list
  +73` passes with the known notice; `moon test --target native` reports
  2428/2428.~~
...
- [x] ~~Continue source-port hardening with a page-label letter performance
  ...
  growing the output array, preserving uppercase and lowercase letter labels,
  prefixed visible labels, roman labels, and prefix-only labels. Validation on
  MoonBit compiler 0.9.3: `moon check --target native --warn-list +73` passes
...
- [x] ~~Continue source-port hardening with an add-text URL markup output
  performance slice: `pdf_addtext_get_urls_line` now validates and counts the
  stripped visible text length up front, allocates that exact byte length, and
  fills URL marker text and literal bytes by index instead of growing the
  output array, preserving visible text, URL ordering, byte-offset spans,
  UTF-8 handling, and malformed marker errors. Validation on MoonBit compiler
  0.9.3: `moon check --target native --warn-list +73` passes with the known
  notice; focused URL-markup tests report 1/1, 1/1, 1/1, and 1/1; `moon test
  --target native pdf_addtext_test.mbt` reports 118/118; `moon check --target
  all --warn-list +73` passes with the known notice; `moon test --target
  native` reports 2430/2430.~~
- [x] ~~Continue source-port hardening with an add-text fontpack run-byte
  performance slice: `pdf_addtext_fontpack_text_ops` now allocates each
  fontpack `Tj` run to the exact run length and fills charcode bytes by index
  instead of growing a temporary byte array, preserving font switching,
  skipped missing glyphs, fallback font names, and emitted text runs.
  Validation on MoonBit compiler 0.9.3: `moon check --target native
  --warn-list +73` passes with the known notice; focused fontpack run tests
...
- [x] ~~Continue source-port hardening with an add-text line splitting output
  ...
  118/118; `moon check --target all --warn-list +73` passes with the known
  notice; `moon test --target native` reports 2430/2430.~~
- [x] ~~Continue source-port hardening with a draw fontpack run-byte
  performance slice: `PdfDrawRenderState::text_ops_and_width_with_cache` now
  allocates each fontpack `Tj` run to the exact run length and fills charcode
  bytes by index instead of growing a temporary byte array, preserving
  document-aware fontpack resource names, font switching, emitted text runs,
  skipped missing glyphs, and measured text width. Validation on MoonBit
  compiler 0.9.3: `moon check --target native --warn-list +73` passes with the
  known notice; focused draw fontpack run tests report 1/1; `moon test --target
  native pdf_draw_test.mbt` reports 56/56; `moon check --target all
  --warn-list +73` passes with the known notice; `moon test --target native`
  reports 2430/2430.~~
- [x] ~~Continue source-port hardening with a draw adjacent-text merge
  performance slice: `pdf_draw_merge_adjacent_tjs` now scans each contiguous
  `Tj` run for the exact combined byte length, allocates that output once, and
  copies fragment bytes by index instead of growing a temporary byte array,
  preserving paragraph cleanup, duplicate-font removal, standard-font space
  bytes, and merged emitted text operations. Validation on MoonBit compiler
  0.9.3: `moon check --target native --warn-list +73` passes with the known
  notice; focused adjacent text merge tests report 1/1; `moon test --target
...
- [x] ~~Continue source-port hardening with a font-listing text output
  ...
  once, counts each cpdf plain-text row exactly, allocates that output length,
  and fills page/resource/subtype/basefont/encoding fields by index instead of
  growing the output array, keeping JSON-mode output, wrapper output, and
  invalid-page behavior stable. Validation on MoonBit compiler 0.9.3: `moon
  check --target native --warn-list +73` passes with the known notice; focused
  font listing tests report 1/1; `moon test --target native pdf_font_test.mbt`
  reports 9/9; `moon check --target all --warn-list +73` passes with the known
...
- [x] ~~Continue source-port hardening with a font-table text output
  ...
  entries once, counts each cpdf encoding-table row exactly, allocates that
  output length, and fills code, Unicode, UTF-8, Unicode-name, and glyph-name
  fields by index instead of growing the output array, keeping wrapper output
  and invalid-page behavior stable. Validation on MoonBit compiler 0.9.3: `moon
  check --target native --warn-list +73` passes with the known notice; focused
...
- [x] ~~Continue source-port hardening with a raw image Decode-array output
  ...
  passes with the known notice; `moon test --target native` reports
  2431/2431.~~
- [x] ~~Continue source-port hardening with a raw image RGB24 conversion output
  allocation slice: `pdf_image_gray_8bpp_as_rgb24` and
  `pdf_image_cmyk_8bpp_as_rgb24` now allocate the exact `pixels * 3` RGB byte
  output and fill component triples by index instead of growing temporary
  arrays, preserving raw DeviceGray and DeviceCMYK `get_image_24bpp` output.
  Validation on MoonBit compiler 0.9.3: `moon check --target native
  --warn-list +73` passes with the known notice; focused raw RGB/gray/CMYK
  image tests report 1/1; `moon test --target native pdf_image_test.mbt`
  reports 55/55; `moon check --target all --warn-list +73` passes with the
  known notice; `moon test --target native` reports 2431/2431.~~
- [x] ~~Continue source-port hardening with a packed grayscale image RGB24
  output allocation slice: `pdf_image_1bpp_as_rgb24` and
  `pdf_image_gray_packed_bpp_as_rgb24` now allocate the exact `width * height *
  3` output length and fill RGB triples by index instead of growing temporary
  arrays, preserving image masks, packed DeviceGray/CalGray 1/2/4bpp samples,
  and `get_image_24bpp` output. Validation on MoonBit compiler 0.9.3: `moon
  check --target native --warn-list +73` passes with the known notice; focused
  packed grayscale image tests report 1/1; `moon test --target native
  pdf_image_test.mbt` reports 55/55; `moon check --target all --warn-list +73`
  passes with the known notice; `moon test --target native` reports
  2431/2431.~~
...
- [x] ~~Continue source-port hardening with indexed RGB/CMYK image output
  ...
  passes with the known notice; `moon test --target native` reports
  2431/2431.~~
- [x] ~~Continue source-port hardening with indexed Lab image output allocation
  slice: indexed Lab 8bpp/packed conversion paths now allocate exact `pixels *
  3` RGB output buffers and write converted Lab table RGB triples by index
  instead of growing temporary arrays, preserving Lab white-point conversion,
  packed sample extraction, and `get_image_24bpp` output. Validation on MoonBit
  compiler 0.9.3: `moon check --target native --warn-list +73` passes with the
  known notice; focused Lab indexed image tests report 1/1; `moon test --target
...
- [x] ~~Continue source-port hardening with TrueType subset font assembly
  ...
  --target all --warn-list +73` passes with the known notice; `moon test
  --target native` reports 2432/2432.~~
- [x] ~~Continue source-port hardening with lexeme string allocation slice:
  `lex_pdf_literal_string` and `lex_pdf_hex_string` now pre-scan their token
  spans, allocate bounded output buffers once, and write decoded bytes by index
  instead of growing temporary arrays, preserving nested literal strings,
  escape handling, odd hex nibbles, malformed hex errors, and dictionary
  delimiter guards. Validation on MoonBit compiler 0.9.3: `moon check --target
  native --warn-list +73` passes with the known notice; `moon test --target
  native pdf_lexeme_test.mbt` reports 17/17; `moon check --target all
  --warn-list +73` passes with the known notice; `moon test --target native .`
  reports 2010/2010; `moon info` and `moon fmt` completed with no `.mbti`
...
- [x] ~~Continue source-port hardening with add-text selection tuning:
  selected-font add-text stamping and rectangle stamping now build one
  `PdfNumberSet` per operation, preallocate rewritten page arrays from page
  count, and remove the last private linear selected-page helper while keeping
  existing page-number validation and empty-selection behavior. Validation on
  MoonBit compiler 0.9.3: `moon check --target native --warn-list +73` passes
...
- [x] ~~Continue source-port hardening with page-resource renumbering and
  ...
  reports 2010/2010.~~
- [x] ~~Continue source-port hardening with page extraction and reference
  renumbering allocation tuning: selected page/object-number lists, annotation
  number scans, duplicate-annotation tables, live name-tree and catalog
  destinations, page-tree node/reference scans, first-kid rewrite outputs,
  deleted-page lists, recursive reference-renumbered arrays/dictionaries, and
  page-reference change tables now preallocate from page, annotation, name-tree,
  dictionary, kids, object, or change counts while preserving extraction,
  duplicate-page repair, annotation-link repair, destination pruning, and
  reference-renumbering behavior. Validation on MoonBit compiler 0.9.3:
  `moon check --target native --warn-list +73` and `moon check --target all
  --warn-list +73` pass with the known notice; `moon test --target native
  pdf_page_test.mbt pdf_page_wbtest.mbt pdf_merge_test.mbt
