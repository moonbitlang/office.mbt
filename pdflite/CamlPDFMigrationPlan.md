...
7. Stream filters and predictors.
   ...
   accounting instead of native miniz. Single-stage Flate inline images without
   `/DecodeParms` now keep that prefix-decoded payload and only advance the
   cursor over the encoded slice, avoiding an encoded-byte copy plus a second
   stream-filter decode. Owned `/Crypt` identity filter
