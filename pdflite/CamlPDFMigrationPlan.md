...
5. Minimal PDF reader.
   ...
   correcting `/Length`; reconstruction scans now reuse scan-local
   `ByteCursor` instances across candidate offsets for xref, object, trailer,
   and xref-stream discovery instead of allocating one per candidate byte.
   Reconstructed object tokenization now skips raw
...
7. Stream filters and predictors.
   ...
   identity predictor `1`. PNG predictor decode now reuses row buffers,
   fixed-predictor PNG encode paths append scanlines directly into the output
   array, and Optimum predictor scoring avoids temporary candidate row arrays
   while preserving first-minimum filter selection.
...
9. Text, fonts, color spaces, and images.
   ...
   generated GB2312 mapping-table fallback with matching reverse lookup.
   GBpc-EUC predefined-CMap extraction now has a generated `GBpc-EUC-UCS2`
   ...
   expansion and reverse lookup for single-scalar entries. HKSCS
   ...
   signed four-byte GBK2K charcodes. Supported Adobe-GB1, Adobe-CNS1, and
   Adobe-Korea1 vertical
...
- decode GBpc-EUC no-`/ToUnicode` predefined-CMap text through a generated
  `GBpc-EUC-UCS2` mapping table, including PC single-byte behavior,
...
- decode KSC-EUC no-`/ToUnicode` predefined-CMap text through a generated
...
- decode HKSCS no-`/ToUnicode` predefined-CMap text through a generated
...
- decode GBK2K no-`/ToUnicode` predefined-CMap text through generated CID
  ranges over shared Adobe-GB1 Unicode data, including two-byte and four-byte
  charcode segmentation, packed signed-`Int` four-byte charcodes, reverse
  charcode lookup, supplementary Unicode scalar output, and invalid-code
  fallback;
