...
| WDP1: document-property inventory | Inventory exactly the OfficeCLI-addressable core keys `title`, `author` (`creator` alias), `subject`, `description`, `keywords`, `category`, `lastModifiedBy`, and `revisionNumber`; report `created`/`modified` as read-only timestamps; and inventory custom properties only as bounded `string`, `bool`, signed-32-bit integer, finite-double, or UTC-date values. Preserve package provenance and emit typed residuals for every other core/custom form. | D2 |
| WDP2: core-property lifecycle | Author, update, or clear exactly the eight mutable WDP1 core keys in fresh and existing documents, canonicalizing `creator` to `author`; `created` and `modified` remain read-only/save-managed, and no application-derived value is fabricated. | A4, D1, D3, WDP1 |
| WDP3: custom-property lifecycle | Add, update, or remove only the five WDP1 custom value kinds with case-insensitive name uniqueness, format-id/pid uniqueness, finite/range validation, and unrelated-property preservation; unsupported VT forms remain inventoried residuals rather than coerced values. | A4, D1, D3, WDP1 |
| WDP4: document-property exposure | Expose WDP1-WDP3 through shared help, create/batch, typed readback, and feature-specific receipts. Typed semantic dump/replay carries the stable core subset (`title`, `author`, `subject`, `description`, `keywords`, `category`) plus lossless WDP3 values; `lastModifiedBy`, `revisionNumber`, `created`, and `modified` are reported as save-managed residuals unless an explicit whole-part raw carrier preserves `/docProps/core.xml`. | A2, A5, R1, WDP2, WDP3 |
...
| X9a1: workbook document-property engine | Inventory and add lifecycle for exactly the OfficeCLI-addressable core keys `title`, `author` (`creator` alias), `subject`, `description`, `keywords`, `category`, `lastModifiedBy`, and `revisionNumber`, with `created`/`modified` read-only; support custom properties only for bounded `string`, `bool`, signed-32-bit integer, finite-double, and UTC-date values with pid/fmtid uniqueness. Preserve unrelated properties and report every unsupported/application-derived form. | X3 |
| X9a2: workbook document-property exposure | Expose X9a1 through workbook get/query, create/batch, shared help, and property-specific receipts. Typed semantic dump/replay carries `title`, `author`, `subject`, `description`, `keywords`, `category`, and lossless custom values; it reports `lastModifiedBy`, `revisionNumber`, `created`, and `modified` as save-managed residuals instead of promising a false fixpoint, while explicit A5 whole-part raw replay remains available. | A5, X4a2, X9a1 |
