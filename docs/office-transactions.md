...
`moonbitlang/office-lib/transaction` is the single publication boundary for future
...
text and parser structures. Strict normalization and both strict and tolerant
entity decoding operate over borrowed ranges, then allocate one exactly sized
retained UTF-16 result; raw, normalized, entity-substring, and growable
token-sized copies therefore cannot consume the reserve concurrently. The DOCX
annotation scanner materializes only namespace bindings and the qualified WML
`id`, `type`, and `val` attributes it consumes, with an explicit per-value bound
before UTF-8 decoding. Before allocation, the splice layer also
