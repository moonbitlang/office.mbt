...
| Locate literal text with editability judgments | `office find FILE --text TEXT [--in PATH] [--limit N] [--json]` |
...
| Apply direct character formatting | `office format FILE OUT.docx (--text TEXT [--nth K] \| --range A:B --in P) [--bold on\|off] [--italic on\|off] [--underline on\|off] [--color RRGGBB] [--expect N] [--allow-zero] [--dry-run] [--overwrite] [--json]` |
